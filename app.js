...
(function () {
  ...
  // --- State ---
  let isPlaying = false;
  let playAbort = null; // AbortController for stopping playback
  let audioCtx = null;

  // Tap decode state
  let tapDown = 0;
  let tapSymbols = ""; // current letter symbols (dots/dashes)
  let tapLetters = []; // decoded letters
  let tapCurrentMorse = []; // morse codes for display
  ...
  if (hasVibration) {
    hapticStatus.textContent = "Haptics supported!";
    hapticStatus.className = "supported";
  } else {
    hapticStatus.textContent = "Haptics not supported on this device.";
    hapticStatus.className = "unsupported";
  }
  ...
  function renderMorse(morseStr, highlightIndex) {
    ...
    // Split into tokens, track which token index each letter is
  ...
  function getUnitMs() {
    // PARIS standard: 50 units = 1 word
    // At wpm W, unit = 1200/W ms
  ...
  async function playMorse() {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Each symbol in the letter generates an "on" event
      // plus intra-character "off" events
    ...
    try {
      for (let i = 0; i < timeline.length; i++) {
        ...
        if (ev.type === "on") {
          ...
          if (hasVibration) navigator.vibrate(durationMs);
  ...
  function renderTapMorse() {
    ...
    // Add current in-progress symbols
    if (tapSymbols) {
  ...
  function onTapEnd(e) {
    ...
    if (duration < DOT_THRESHOLD) {
      ...
      tapSymbols += "-";
      if (hasVibration) navigator.vibrate(80);
    }

    renderTapMorse();

    // Set timeout for letter boundary (600ms pause = end of letter)
    letterTimeout = setTimeout(() => {
      finishLetter();
      // Set timeout for word boundary (additional 800ms = space)
  ...
  function buildReference() {
    ...
    // Group: letters first, then numbers, then symbols
    ...
    all.forEach((char) => {
      ...
      // Tap a reference card to play it
